...
let initial_ctx {
  ...
  add "fst" (t_tuple [a; b] @-> a);
  add "snd" (t_tuple [a; b] @-> b);
  ...
  (
    ...
    add "ref"   (a @-> t_ref a);
    add "deref" (t_ref a @-> a);
    add ":="    (t_ref a @-> a @-> t_unit)
    );
  add_con "::" [a; t_list a] (t_list a);
  (* TODO: remove this; have AST treat (::) in expr as a constructor *)
  add     "::" (a @-> t_list a @-> t_list a);
  add     "@"  (t_list a @-> t_list a @-> t_list a);
  let t_option = ty1 "option" in
  add_con "None" []  (t_option a);
  add_con "Some" [a] (t_option a);
  add_mod "Option" (fun { add; _ } ->
    add "map"    ((a @-> b) @-> (t_option a @-> t_option b));
    add "unwrap" (t_option a @-> a);
    add "bind"   (t_option a @-> (a @-> t_option b) @-> t_option b);
    ()
  );
  add_mod "List" (fun { add; _ } ->
    add "init"       (t_int @-> (t_int @-> a) @-> t_list a);
    add "rev"        (t_list a @-> t_list a);
    add "fold_left"  ((a @-> b @-> a) @-> (a @-> t_list b @-> a));
    add "fold_right" ((b @-> a @-> a) @-> (t_list b @-> a @-> a));
    add "map"        ((a @-> b) @-> (t_list a @-> t_list b));
    add "map2"       ((a @-> b @-> c) @-> (t_list a @-> t_list b @-> t_list c));
    add "mapi"       ((t_int @-> a @-> b) @-> (t_list a @-> t_list b));
    add "for_all"    ((a @-> t_bool) @-> t_list a @-> t_bool);
    add "filter"     ((a @-> t_bool) @-> (t_list a @-> t_list a));
    add "find_opt"   ((a @-> t_bool) @-> (t_list a @-> t_option a));
    add "iter"       ((a @-> t_unit) @-> (t_list a @-> t_unit));
    add "length"     (t_list a @-> t_int);
    add "concat"     (t_list (t_list a) @-> t_list a);
    add "concat_map" ((a @-> t_list b) @-> (t_list a @-> t_list b));
    ()
  );
  let t_result = ty2 "result" in
  add_con "Ok"    [a] (t_result a b);
  add_con "Error" [b] (t_result a b);
  ...
  add_mod "StringMap" (fun { add; add_con; ty0; ty1; _ } ->
    let t = ty1 "t" in
    add "empty"     (t a);
    add "singleton" (t_string @-> a @-> t a);
    add "lookup"    (t_string @-> t a @-> t_option a);
    add "eql"       ((a @-> a @-> t_bool) @-> (t a @-> t a @-> t_bool));
    add "insert"    (t_string @-> a @-> t a @-> t_option (t a));
    add "map"       ((t_string @-> a @-> b) @-> (t a @-> t b));
    add "fold"      ((a @-> t_string @-> b @-> a) @-> (a @-> t b @-> a));
    let t_dup_err = ty0 "dup_err" in
    add_con "DupErr" [t_string] t_dup_err;
    add "disjoint_union" (t a @-> t a @-> t_result (t a) t_dup_err);
    ()
  );
  add_mod "IntMap" (fun { add; ty1; _ } ->
    let t = ty1 "t" in
    add "empty"    (t a);
    add "is_empty" (t a @-> t_bool);
    add "lookup"   (t_int @-> t a @-> t_option a);
    add "insert"   (t_int @-> a @-> t a @-> t a);
    add "fold"     ((a @-> t_int @-> b @-> a) @-> (a @-> t b @-> a));
    add "union"    (t a @-> (t a @-> t a));
    add "iter"     ((t_int @-> a @-> t_unit) @-> t a @-> t_unit);
    add "filter"   ((t_int @-> a @-> t_bool) @-> t a @-> t a);
    ()
  );
